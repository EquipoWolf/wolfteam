...
SOURCES = [{"url": "https://googledrive.com/host/0B0BObI90Md8oejVrUFRzbTdINGs", "fanart": "https://googledrive.com/host/0B0BObI90Md8oYkVpRUhpM1FLS2M/fanart.jpg", "genre": "Tv Live", "date": "11.01.2015", "credits": "yulfiostv", "thumbnail": "https://googledrive.com/host/0B0BObI90Md8oYkVpRUhpM1FLS2M/icon.png"}]

# http://i.imgur.com/41ZDC18.jpg
...
def getSources():
        if os.path.exists(favorites) == True:
            ...
            if FAV == "[]":
                ...
                addDir('[COLOR cyan][B]- FAVORITES CHANNEL Yulfiostv [/COLOR][/B][/B]','url',4,os.path.join(home, 'resources', 'favorite.png'),FANART,'','','','')
                addDir('','',100,'',FANART,'','','','')
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(Yulfiostv,Urlresolver donot support this domain. - ,5000)")
    return resolver
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie="",allinfo={}):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from Yulfiostv Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to Yulfiostv Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
#            ln+='</item>'
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(Yulfiostv,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(Yulfiostv,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(Yulfiostv,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(Yulfiostv,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
